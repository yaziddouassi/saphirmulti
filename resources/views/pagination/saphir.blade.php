    ...
    x-data="">
        <div class="flex justify-between flex-1 sm:hidden">
            ...
            @if ($paginator->hasMorePages())
                <button @click="$wire.groupId=[]"
                wire:click="nextPage" class="ml-3 relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
                    Next 
                </button>
            @else
                <span @click="$wire.groupId=[]"
                ...
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span @click="$wire.groupId=[]" aria-disabled="true">
                                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-[#ddd] border border-gray-300 cursor-default leading-5">{{ $element }}</span>
                            </span>
                        @endif
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span @click="$wire.groupId=[]" aria-current="page">
                                        <span 
                                         class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium bg-[blue] text-white border border-gray-300 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
                                    <button wire:click="gotoPage({{ $page }})" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-[#E1E1E1] border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="{{ $page }}"
                                    @click="$wire.groupId=[]">
                                        {{ $page }}
                                    </button>
                                @endif
